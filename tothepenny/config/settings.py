...
try:  # a .env file is a convenience on a desktop; the browser build has none
...
# TOTHEPENNY_OUTPUT_DIR / TOTHEPENNY_LOG_DIR) and are created only when something is written.
PACKAGE_DIR = Path(__file__).resolve().parent.parent
BANK_TEMPLATES_DIR = PACKAGE_DIR / "bank_templates"
OUTPUT_DIR = Path(os.getenv("TOTHEPENNY_OUTPUT_DIR", Path.cwd() / "output"))
LOGS_DIR = Path(os.getenv("TOTHEPENNY_LOG_DIR", Path.cwd() / "logs"))
