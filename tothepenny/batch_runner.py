...
def combined_rows(summary: BatchRunSummary) -> tuple:
    ...
    statement supplied twice is counted once (the coverage report lists duplicates). Transfers between accounts in
    the set are matched and each side names the other (see transfers.py). Returns (rows, transfer pairs)."""
    from .transfers import cite, match_transfers
...
def write_all_transactions_csv(summary: BatchRunSummary, path: Path, transfers_path: Optional[Path] = None,
                               combined: Optional[tuple] = None) -> tuple:
    """The combined dataset as all_transactions.csv, and the matched transfers as transfers.csv. Returns
    (transactions written, transfers matched)."""
    import csv
    from .transfers import write_transfers_csv
    rows, pairs = combined or combined_rows(summary)
...
def finish_batch(summary: BatchRunSummary, output_dir: Path) -> dict:
    """Everything a batch produces beyond each statement's own file, the same from the command line and the browser:
    batch_report.csv (every file and whether it was proved), all_transactions.csv and transfers.csv (the combined
    dataset), coverage_report.csv (whether each account's statements join up) and statements.xlsx (all four in one
    workbook)."""
    from .coverage import account_coverage, write_coverage_csv
    from .exporters.set_workbook import write_set_workbook
    output_dir.mkdir(parents=True, exist_ok=True)
    write_batch_report_csv(summary, output_dir / 'batch_report.csv')
    rows, pairs = combined_rows(summary)
    write_all_transactions_csv(summary, output_dir / 'all_transactions.csv', output_dir / 'transfers.csv',
                               combined=(rows, pairs))
    findings = account_coverage(summary.results)
    write_coverage_csv(findings, output_dir / 'coverage_report.csv')
    write_set_workbook(summary, rows, pairs, findings, output_dir / 'statements.xlsx')
    return {'transactions': len(rows), 'transfers': len(pairs), 'findings': findings}
