...
def _page_chars(textpage, page_height: float) -> List[Tuple[str, float, float, float, float, float]]:
    ...
    def entries():
        for index in range(max(textpage.count_chars(), 0)):
            code, generated = get_unicode(handle, index), is_generated(handle, index)
            if get_box(handle, index, box):
                yield (code, generated, box.left, box.right, box.top, box.bottom, get_size(handle, index))
            else:
                yield (code, generated)
    return _chars_from_entries(entries(), page_height)


def _chars_from_entries(entries, page_height: float) -> List[Tuple[str, float, float, float, float, float]]:
    """Characters from PDFium's per-character entries, (code, generated, left, right, top, bottom, size) or
    (code, generated) where it gives no box, whether read here by pypdfium2 or in a browser by PDFium compiled to
    WebAssembly."""
    chars = []
    for entry in entries:
        code, generated = entry[0], entry[1]
        if not code or (code in (32, 13, 10) and generated == 1) or len(entry) < 7:
            continue  # pdfium's inferred spaces and line breaks are left out: word breaks are decided below
        left, right, top, bottom, size = entry[2:7]
        if 0xDC00 <= code <= 0xDFFF and chars and len(chars[-1][0]) == 1 and 0xD800 <= ord(chars[-1][0]) <= 0xDBFF:
            ...
            chars.append((pair, high[1], max(high[2], right), high[3], high[4], high[5]))
            continue
        chars.append((chr(code), left, right, page_height - top, page_height - bottom, size))
...
def _rules(page, page_height: float):
    ...
    import pypdfium2.raw as raw
    try:
        paths = [obj.get_pos() for obj in page.get_objects() if obj.type == raw.FPDF_PAGEOBJ_PATH]
    except Exception:  # noqa: BLE001 - rules are a help, never a requirement
        return (), ()
    return _rules_from_paths(paths, page_height)


def _rules_from_paths(paths, page_height: float):
    """Thin lines from path bounds (left, bottom, right, top); None when the paths could not be read."""
    if paths is None:
        return (), ()
    vertical, horizontal = [], []
...
# Pages read elsewhere (in a browser, by PDFium compiled to WebAssembly), keyed by the file's resolved path
_PRELOADED: dict = {}
...
def reading(pdf_path) -> dict:
    """A file read with pypdfium2 into the form the browser reader (web/pdf-read.js) produces, for tests and for
    checking that the two readings agree."""
    ...
    try:
        ...
        for index in range(len(document)):
            ...
            box = raw.FS_RECTF()
            chars = []
            for i in range(max(textpage.count_chars(), 0)):
                code, generated = raw.FPDFText_GetUnicode(textpage.raw, i), raw.FPDFText_IsGenerated(textpage.raw, i)
                if raw.FPDFText_GetLooseCharBox(textpage.raw, i, box):
                    chars.append([code, generated, box.left, box.right, box.top, box.bottom,
                                  raw.FPDFText_GetFontSize(textpage.raw, i)])
                else:
                    chars.append([code, generated])
            try:
                paths = [list(obj.get_pos()) for obj in page.get_objects() if obj.type == raw.FPDF_PAGEOBJ_PATH]
            except Exception:  # noqa: BLE001 - as the desktop reader: no rules for the page
                paths = None
            pages.append({'width': width, 'height': height, 'chars': chars, 'paths': paths,
...
def _read(path: str, mtime: float) -> Tuple[Page, ...]:
    ...
    if data is not None:
        ...
        for index, page in enumerate(data['pages']):
            ...
            chars = _chars_from_entries(page['chars'], height)
            vertical, horizontal = _rules_from_paths(page.get('paths'), height)
